    ...
    def test_hca_extraction(self):
        ...
        for aggregate in True, False:
            with self.subTest(aggregate=aggregate):
                ...
                total_projects = self.opensearch.count(index=index_name('projects'))
                # Three unique projects, six project contributions
                self.assertEqual(3 if aggregate else 6, total_projects['count'])
                total_files = self.opensearch.count(index=index_name('files'))
                self.assertEqual(776, total_files['count'])
                total_samples = self.opensearch.count(index=index_name('samples'))
            ...
            with self.subTest(aggregate=aggregate):
                ...
                if aggregate:
                    ...
                    coordinates = ContributionCoordinates(entity=entity,
                                                          bundle=bundle_fqid,
                                                          deleted=False)
                result = self.opensearch.get(index=coordinates.index_name,
